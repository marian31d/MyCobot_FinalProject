...
def main():
    ...
    num_contours=1
    crop_tool = vision.Crop(path=input_image_path, n=num_contours)  # `n` is unused here
    # Run the crop tool
    crop_tool.main(input_image_path, cropped_image_path)
     # Detect contours for two objects in the cropped region
    contour_tool = vision.Crop(path=input_image_path, n=num_contours)
    ...
    print(fm[0])
    crds = client.transform([fm[0][0], fm[0][1]], M)
